from ai.service import ask_ai
from memory.store import add_message, get_recent_messages
from services.knowledge_service import learn_knowledge, search_knowledge_base, search_wikipedia
from services.math_service import answer_math


DEFAULT_FALLBACK = (
    "Ainda não encontrei uma resposta confiável. Posso tentar de novo com mais contexto, "
    "consultar minha base local ou registrar uma informação nova."
)
...
def generate_reply(user_id, message):
    ...
    if learned:
        reply = "Aprendizado registrado. Vou reutilizar essa informação quando ela for relevante."
